...
import { updateTaskRequestSchema, updateTaskStatusRequestSchema } from "@onevyrt/contracts";
import {
  updateTask,
  updateTaskStatus,
...
/** The project/priority/blocker fields - kept separate from PATCH above, which only ever changes status (and its completedAt side effect). */
export async function PUT(request: NextRequest, { params }: RouteParams) {
  ...
  const parsed = updateTaskRequestSchema.safeParse(await request.json().catch(() => null));
  if (!parsed.success) {
    return NextResponse.json(
      { error: "Invalid request", issues: parsed.error.flatten() },
      { status: 400 },
